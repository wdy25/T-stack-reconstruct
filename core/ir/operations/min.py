...
class Min(Operation):
	"""Row-wise min reduction implemented with a single MaxPooling primitive.
	...
	def infer(self, inputs: Dict[int, Data]) -> List[Tuple[Optional[Tuple[int, ...]], Optional[DataType]]]:
		if 0 not in inputs:
			raise ValueError("Min expects input tensor on port 0.")

		input_data = inputs[0]
		if input_data.shape is None or len(input_data.shape) != 2:
			raise ValueError("Min input tensor must have shape (vector_num, vector_len).")

		if input_data.dtype != DataType.BF16:
			raise ValueError("Min currently supports BF16 input tensors only.")

		vector_num, vector_len = input_data.shape
		if vector_num <= 0 or vector_len <= 0:
			raise ValueError("Input dimensions must be positive integers for Min operation.")
	...
	def para_node(self, inputs: Dict[int, Data], outputs: Dict[int, Data]) -> Optional[Data]:
		...
		prim_min = self._build_primitive(
		...
		para_code = MemBlock(length=1, payload=[prim_min.para])
