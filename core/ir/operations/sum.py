...
class Sum(Operation):
    """Row-wise sum reduction implemented with a single MeanPooling primitive.
    ...
    def infer(self, inputs: Dict[int, Data]) -> List[Tuple[Optional[Tuple[int, ...]], Optional[DataType]]]:
        if 0 not in inputs:
            raise ValueError("Sum expects input tensor on port 0.")

        input_data = inputs[0]
        if input_data.shape is None or len(input_data.shape) != 2:
            raise ValueError("Sum input tensor must have shape (vector_num, vector_len).")

        if input_data.dtype != DataType.BF16:
            raise ValueError("Sum currently supports BF16 input tensors only.")

        vector_num, vector_len = input_data.shape
        if vector_num <= 0 or vector_len <= 0:
            raise ValueError("Input dimensions must be positive integers for Sum operation.")
    ...
    def para_node(self, inputs: Dict[int, Data], outputs: Dict[int, Data]) -> Optional[Data]:
        input_data = inputs[0]
        prim_sum = self._build_primitive(
        ...
        para_code = MemBlock(length=1, payload=[prim_sum.para])
    ...
    def gen_prim(
        ...
        prim_sum = self._build_primitive(
            ...
            y_out_addr=outputs[0].memref.addr,
