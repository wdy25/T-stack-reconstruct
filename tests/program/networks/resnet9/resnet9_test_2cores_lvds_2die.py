...
from core_components.communication_op import SendOp, SendMsg
...
def main():
    ...
    from zk_Resnet9_data_gen import QuantizedResNet9
    net = QuantizedResNet9(layer_info=extractInt8QuantizedOnnx("test_gen/network/resnet9/MQuantized.onnx"), test_name="testcases/resnet9_lvds_2dies", core_y=1, core_x=1, addr=2780)
    net.getParams()
    net.quantize()
    net.forward(image)
    ...
    hwg.set_core_id_for_nodes(gs[0], core_id=(2, 1))
    hwg.set_core_id_for_nodes(gs[1], core_id=(1, 5))
    # hwg.gen_memref_for_all_data()
    hwg.gen_communication_ops()
    
    send_node = SendOp(name="send_out", attrs={'source': (1, 5), 'dest': (0, -1), 'tag': 0})
    hwg.add_node(send_node, (1, 5))
    hwg.connect("output", "send_out", 0, 0)
    # msg = SendMsg(S=0, T=1, E=0, Q=0, LVDS=0, Y=-1, X=-2, A0=0, pack_per_rhead=0, A_offset=1, Const=0, handshake=0)
    ...
    hwg.node("send_out").configure_send_manual(send_prim_config={
        'deps': 1,
        'send': 1,
        'send_addr': 0,
        'cell_or_neuron': 0,
    ...
    hwg.visualize("testcases/resnet9_lvds_2dies/resnet9_hardware_graph", format="pdf", vertical=True)
    allocator.visualize_lifecycle("testcases/resnet9_lvds_2dies/resnet9_memory_lifecycle")
    ...
    config_gen = ConfigGenerator(hwg, codes, output_dir="testcases/resnet9_lvds_2dies/")
