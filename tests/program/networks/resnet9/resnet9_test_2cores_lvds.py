...
def main():
    ...
    net = QuantizedResNet9(layer_info=extractInt8QuantizedOnnx("test_gen/network/resnet9/MQuantized.onnx"), test_name="testcases/resnet9_lvds", core_y=1, core_x=1, addr=2780)
    net.getParams()
    net.quantize()
    net.forward(image)
    ...
    hwg.set_core_id_for_nodes(gs[1], core_id=(1, 1))
    # hwg.gen_memref_for_all_data()
    hwg.gen_communication_ops()
    
    send_node = SendOp(name="send_out", attrs={'source': (1, 1), 'dest': (0, -1), 'tag': 0})
    hwg.add_node(send_node, (1, 1))
    ...
    hwg.visualize("testcases/resnet9_lvds/resnet9_hardware_graph", format="pdf", vertical=True)
    allocator.visualize_lifecycle("testcases/resnet9_lvds/resnet9_memory_lifecycle")
    ...
    config_gen = ConfigGenerator(hwg, codes, output_dir="testcases/resnet9_lvds/")
