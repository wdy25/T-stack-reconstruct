...
def main():
    ...
    layer_info = extractInt8QuantizedOnnx("tests/program/networks/resnet9/MQuantized.onnx")
    # Load sample input (already tensor NCHW)
    image = torch.load("tests/program/networks/resnet9/input_data/truck.tensor")
    g = construct_resnet9_graph(layer_info, image)

    hwg = HardwareGraph(g)
    hwg.gen_memref_for_all_data()
    hwg.set_core_id_for_nodes(hwg.all_nodes(), core_id=(0, 0))
    ...
    hwg.visualize("testcases/resnet9_single_core/resnet9_hardware_graph", format="pdf", vertical=True)
    allocator.visualize_lifecycle("testcases/resnet9_single_core/resnet9_memory_lifecycle")
    ...
    config_gen = ConfigGenerator(hwg, codes, output_dir="testcases/resnet9_single_core/")
