...
from tests.utils.soma.utils import make_input_payload_for_lut, compute_expected_torch_lut
...
def construct_lut_graph(
    ...
    y_true = compute_expected_torch_lut(x, out_dtype, function)
...
def run_one_case(in_dtype: DataType, out_dtype: DataType, shape: tuple[int, int], function: str) -> None:
    graph, true_output = construct_lut_graph(in_dtype, out_dtype, shape, function)
    ...
    if out_dtype == DataType.INT8:
        ...
        assert torch.equal(out, true_output), (
            f"Mismatch for LUT({function}): {in_dtype} -> {out_dtype}, shape={shape}"
        )
    else:
        assert out.dtype == torch.bfloat16
        assert torch.equal(out.view(torch.int16), true_output.view(torch.int16)), (
            f"Bitwise mismatch for LUT({function}): {in_dtype} -> {out_dtype}, shape={shape}"
        )
...
if __name__ == "__main__":
    ...
    for function in functions:
        for in_dtype, out_dtype, shape in combos:
            print(f"Running LUT emulator case: func={function}, {in_dtype} -> {out_dtype}, shape={shape}")
            run_one_case(in_dtype, out_dtype, shape, function)

    print("All LUT emulator cases passed.")
