...
def handwritten_int8_conv(padded_x: torch.Tensor,w: torch.Tensor,b: torch.Tensor,y: torch.Tensor, sh, sw, dh, dw):
...
def construct_deepconv_graph() -> Graph:
    ...
    if conv_type == 'INT8':
        true_output = handwritten_int8_conv(padded_x_in,w_in,b_in,y_out,stride_h,stride_w,dilation_h,dilation_w)
    elif conv_type == 'BF16':
...
if __name__ == "__main__":
    graph, true_output = construct_deepconv_graph()
    ...
    outputs = core_array.get_outputs(get_output_list(hardware_graph, ["DeepConv_output"]))
    print(torch.sum(torch.abs(outputs["DeepConv_output"] - true_output.permute(0,2,3,1))))
    
    print("DeepConv operation graph constructed and processed successfully.")