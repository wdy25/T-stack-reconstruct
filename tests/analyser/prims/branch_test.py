...
from core.ir.operations.branch import Branch
...
def construct_branch_graph(condition_value=1) -> Graph:
    """
    构建包含 Branch 操作的计算图
    condition_value: 条件值，大于 0 时执行跳转，否则继续执行
    """
    graph = Graph()
    ...
    condition_data = torch.tensor([condition_value], dtype=torch.int32)
    ...
    condition_input = Data(name="condition", dtype=DataType.INT32, shape=(1,), payload=condition_data)
    ...
    # 创建 Branch 操作
    branch_op = Branch("branch_op", {"jump_addr": -2, "relative": 1})
    ...
    graph.add_node(condition_input)
    ...
    graph.connect(condition_input.name, branch_op.name)
    
    # 连接控制边
    graph.connect_control(add_op.name, branch_op.name)
    graph.connect_control(branch_op.name, add_op.name)  # 条件为真时跳回 Add 操作
    graph.connect_control(branch_op.name, stop_op.name)  # 条件为假时跳转到 Stop 操作
...
if __name__ == "__main__":
    print("Testing Branch operation with condition > 0:")
    graph = construct_branch_graph(condition_value=1)
    ...
    print("Branch operation with condition > 0 completed.")
    
    # 测试条件为假的情况
    print("\n\nTesting Branch operation with condition <= 0:")
    graph = construct_branch_graph(condition_value=0)
    ...
    print("\nBranch operation tests completed.")
