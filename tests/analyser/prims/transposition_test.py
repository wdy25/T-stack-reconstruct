...
from tests.utils.dendrite.utils import make_input_payload_for_transposition
...
def run_one_case(in_dtype: DataType, shape: tuple[int, int, int, int], transpose_order: str) -> None:
    graph = construct_transposition_graph(in_dtype, shape, transpose_order)
    ...
    print("\nStart running Transposition analyser simulation...")
...
if __name__ == "__main__":
    ...
    for dtype, shape in cases:
        for order in transpose_orders:
            print(f"\nRunning Transposition case: dtype={dtype}, order={order}, shape={shape}")
            run_one_case(dtype, shape, order)

    print("\nAll Transposition analyser cases finished.")